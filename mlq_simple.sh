...
if [[ $1 == "show-all-if-ambiguous" || $# -lt 1 || $# -gt 1 ]] ; then
    echo 'Please give the shortcut name only.'
    echo 'Loaded modules will be saved into the shortcut file '$PWD'/<shortcut>.lua'
    return
fi
shortcut_name="$1"
...
echo 'Shortcut saved to: '"${shortcut_name}".lua
echo 'Load with: '
echo 'ml reset; ml use '$PWD' ; ml '"${shortcut_name}"
